...
export default function App() {
  ...
  // Default to dark mode (Auditory Mode dark theme feel). When light, Visual Mode light theme feel.
  ...
  // Toggle dark/light theme on root attribute
  useEffect(() => {
  ...
  return (
    <div className={`min-h-screen font-sans transition-colors duration-500 relative overflow-x-hidden selection:bg-[#0A44FF] selection:text-white ${
      isDark ? 'bg-[#1C1C1E] text-gray-200' : 'bg-[#F8F9FC] text-gray-900'
    }`}>
      {/* Exact Authentic Sensa Keyframes from ModeSelection.tsx */}
      <style dangerouslySetInnerHTML={{
        ...
        .animate-float-blue { animation: float-blue 9s ease-in-out infinite; }
        .animate-float-orange { animation: float-orange 9s ease-in-out infinite 0.5s; }
        .animate-logo-light { animation: logo-light-flow 4.5s ease-in-out infinite; }
        .animate-pop { animation: pop-in 0.6s cubic-bezier(0.23,1,0.32,1) forwards; }
      `}} />

      {/* Background Floating Orbs (`ModeSelection.tsx` authenticity) */}
      <div className={`fixed -top-32 -left-32 w-[550px] h-[550px] rounded-full mix-blend-screen filter blur-[110px] animate-float-blue pointer-events-none -z-10 ${
        isDark ? 'bg-[#0A44FF]/22' : 'bg-[#0A44FF]/14'
      }`} />
      <div className={`fixed -bottom-32 -right-32 w-[550px] h-[550px] rounded-full mix-blend-screen filter blur-[110px] animate-float-orange pointer-events-none -z-10 ${
        isDark ? 'bg-[#FF7A2F]/22' : 'bg-[#FF7A2F]/14'
      }`} />

      {/* ======================================================================
          NAVBAR SLOT: Drop your saved Navbar component code right below this line
          ====================================================================== */}
      <header className={`sticky top-0 z-50 w-full backdrop-blur-md border-b px-4 md:px-12 py-4 transition-all duration-300 ${
        isDark 
        ...
        <div className="w-full max-w-[1800px] mx-auto flex items-center justify-between gap-8">
          <a href="#hero" className="flex items-center gap-3.5 no-underline group shrink-0">
            ...
            />
            <span className={`text-2xl md:text-3xl font-black tracking-tight transition-colors ${
              isDark ? 'text-white group-hover:text-[#6AA2FF]' : 'text-gray-900 group-hover:text-[#0A44FF]'
            }`}>
              Sensa
            </span>
          </a>

          <ul className={`hidden lg:flex items-center justify-around flex-1 max-w-5xl mx-8 list-none m-0 p-0 text-sm font-extrabold tracking-wide ${
          ...
          <div className="flex items-center shrink-0">
            {/* Theme Toggle Switch (Light Theme / Dark Theme) */}
            <button
              onClick={() => setTheme(isDark ? 'light' : 'dark')}
              className={`relative flex items-center h-10 w-36 rounded-full p-1.5 transition-colors duration-300 border shadow-inner focus:outline-none cursor-pointer ${
                isDark
                  ? 'bg-[#24262B] border-[#FF7A2F]/50 text-white'
                  : 'bg-gray-100 border-[#0A44FF]/40 text-gray-900 shadow-sm'
              }`}
              title={isDark ? "Switch to Light Mode" : "Switch to Dark Mode"}
            >
              {/* Animated Sliding Pill Thumb */}
              <div
                className={`absolute top-1 bottom-1 w-[calc(50%-4px)] rounded-full transition-all duration-300 flex items-center justify-center gap-1.5 shadow-md font-extrabold text-xs ${
                  isDark
                    ? 'translate-x-[calc(100%+2px)] bg-[#FF7A2F] text-white shadow-[#FF7A2F]/40'
                    : 'translate-x-0 bg-[#0A44FF] text-white shadow-[#0A44FF]/40'
                }`}
              >
                {isDark ? (
                  <>
              ...
              {/* Background Labels inside Track */}
              <div className="w-full flex items-center justify-between px-3 text-[11px] font-bold z-0 pointer-events-none">
                <span className={`transition-opacity duration-300 flex items-center gap-1 ${!isDark ? 'opacity-0' : 'opacity-80 text-gray-400'}`}>
                  <Sun size={12} /> Light
                </span>
                <span className={`transition-opacity duration-300 flex items-center gap-1 ${isDark ? 'opacity-0' : 'opacity-80 text-gray-600'}`}>
                  <Moon size={12} /> Dark
                </span>
              </div>
            </button>
      ...
      <section id="hero" className="relative pt-16 pb-24 md:pt-24 md:pb-32 px-4 md:px-8">
        <div className="max-w-4xl mx-auto text-center flex flex-col items-center">
          ...
          <p className={`text-base sm:text-lg md:text-xl max-w-3xl mb-10 leading-relaxed font-medium animate-pop ${
          ...
          <div className="flex items-center justify-center w-full mb-4 animate-pop">
            <a 
              href="#guide" 
              className={`inline-flex items-center justify-center gap-2.5 px-8 py-4 rounded-[22px] font-black text-base shadow-xl transition-all duration-200 transform hover:scale-[1.02] active:scale-95 focus:ring-2 focus:ring-[#0A44FF] focus:outline-none ${
                isDark
                  ? 'bg-[#FF7A2F] hover:bg-[#E8671F] text-white shadow-[#FF7A2F]/40'
                  : 'bg-[#0A44FF] hover:bg-[#0035D9] text-white shadow-[#0A44FF]/40'
              }`}
            >
              <Download size={20} /> Add to Chrome — Free ZIP / Extension
            </a>
          </div>
        </div>
      </section>
      ...
      {/* ======================================================================
          ...
          ====================================================================== */}
      <section id="problem-solution" className={`py-20 md:py-28 px-4 md:px-8 max-w-7xl mx-auto border-t ${
        isDark ? 'border-[#3A3F4A]/60' : 'border-gray-200'
      }`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
          {/* The Problem Card */}
          <div className={`backdrop-blur-md border-[2px] border-l-4 border-l-red-500 rounded-[22px] p-6 md:p-8 flex flex-col justify-between shadow-lg transition-all duration-300 hover:scale-[1.02] ${
            isDark 
          ...
          <div className={`backdrop-blur-md border-[2px] border-l-4 border-l-emerald-500 rounded-[22px] p-6 md:p-8 flex flex-col justify-between shadow-lg transition-all duration-300 hover:scale-[1.02] ${
            isDark 
      ...
      {/* ======================================================================
          ...
          ====================================================================== */}
      <section id="features" className={`py-20 md:py-28 px-4 md:px-8 max-w-7xl mx-auto border-t ${
        isDark ? 'border-[#3A3F4A]/60' : 'border-gray-200'
      }`}>
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className={`text-3xl md:text-4xl lg:text-5xl font-black tracking-tight mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>
        ...
        <div className="mb-16">
          <div className="flex items-center gap-3 mb-8">
            <span className="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-[#0A44FF]/20 text-[#6AA2FF] border border-[#0A44FF]/40">
              Mode 1
            </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className={`backdrop-blur-md border-[2px] border-t-4 border-t-[#0A44FF] rounded-[22px] p-6 shadow-md transition-all duration-300 hover:scale-[1.02] ${
              isDark 
              ...
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 border ${
                isDark ? 'bg-[#0A44FF]/22 text-[#6AA2FF] border-[#0A44FF]/40' : 'bg-[#0A44FF]/12 text-[#0A44FF] border-[#0A44FF]/30'
              }`}>
                <Mic size={24} />
              </div>
        ...
        <div>
          <div className="flex items-center gap-3 mb-8">
            <span className="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-[#FF7A2F]/20 text-[#FFC09B] border border-[#FF7A2F]/40">
              Mode 2
            </span>
            <h3 className={`text-2xl md:text-3xl font-black m-0 ${isDark ? 'text-white' : 'text-gray-900'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className={`backdrop-blur-md border-[2px] border-t-4 border-t-[#FF7A2F] rounded-[22px] p-6 shadow-md transition-all duration-300 hover:scale-[1.02] ${
              isDark 
              ...
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 border ${
                isDark ? 'bg-[#FF7A2F]/22 text-[#FFC09B] border-[#FF7A2F]/40' : 'bg-[#FF7A2F]/12 text-[#FF7A2F] border-[#FF7A2F]/30'
              }`}>
                <Ear size={24} />
              </div>
      ...
      {/* ======================================================================
          ...
          ====================================================================== */}
      <section id="playground" className={`py-20 md:py-28 px-4 md:px-8 max-w-7xl mx-auto border-t ${
        isDark ? 'border-[#3A3F4A]/60' : 'border-gray-200'
      }`}>
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className={`text-3xl md:text-4xl lg:text-5xl font-black tracking-tight mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>
        ...
        <div className={`border rounded-[26px] p-4 sm:p-6 md:p-8 shadow-2xl backdrop-blur-md ${
          isDark ? 'bg-[#24262B]/90 border-[#3A3F4A]' : 'bg-white/95 border-[#E2E6F0]'
        }`}>
          {/* Toolbar */}
          <div className={`flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pb-6 border-b mb-6 ${
            isDark ? 'border-[#3A3F4A]' : 'border-gray-200'
          }`}>
            <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
              ...
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-xs ${
                isDark ? 'bg-[#1C1C1E] border-[#3A3F4A]' : 'bg-gray-100 border-gray-300'
              }`}>
          ...
          <div className={`p-5 rounded-[20px] border flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 mb-8 ${
            isDark ? 'bg-[#1C1C1E] border-[#3A3F4A]' : 'bg-gray-50 border-gray-200'
          }`}>
            ...
            <div className="flex items-center gap-2 flex-wrap w-full lg:w-auto">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors border ${
                  isDark ? 'bg-[#24262B] hover:bg-[#0A44FF] text-gray-300 hover:text-white border-[#3A3F4A]' : 'bg-white hover:bg-[#0A44FF] text-gray-700 hover:text-white border-gray-300'
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors border ${
                  isDark ? 'bg-[#24262B] hover:bg-[#FF7A2F] text-gray-300 hover:text-white border-[#3A3F4A]' : 'bg-white hover:bg-[#FF7A2F] text-gray-700 hover:text-white border-gray-300'
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors border ${
                  isDark ? 'bg-[#24262B] hover:bg-emerald-600 text-gray-300 hover:text-white border-[#3A3F4A]' : 'bg-white hover:bg-emerald-600 text-gray-700 hover:text-white border-gray-300'
                }`}
              >
                "Read Page Content"
              </button>
          ...
          <div className={`relative rounded-2xl p-6 md:p-10 border overflow-hidden min-h-[340px] ${
            isDark ? 'bg-[#1C1C1E]/90 border-[#3A3F4A]' : 'bg-gray-50 border-gray-200'
          }`}>
            ...
            <div className="max-w-3xl mx-auto py-2 relative z-0">
              ...
              <div className={`p-4 rounded-xl border text-sm italic ${
                isDark ? 'bg-[#24262B] border-[#3A3F4A] text-gray-400' : 'bg-white border-gray-200 text-gray-600'
              }`}>
          ...
          {(isSimulatingCaptions || activeMode === 'auditory') && (
            <div className={`mt-6 border rounded-2xl p-4 shadow-xl flex items-center justify-between gap-4 animate-pop ${
              isDark ? 'bg-[#1C1C1E] border-[#FF7A2F]/50' : 'bg-white border-[#FF7A2F]'
            }`}>
      ...
      {/* ======================================================================
          ...
          ====================================================================== */}
      <section id="architecture" className={`py-20 md:py-28 px-4 md:px-8 max-w-7xl mx-auto border-t ${
        isDark ? 'border-[#3A3F4A]/60' : 'border-gray-200'
      }`}>
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className={`text-3xl md:text-4xl lg:text-5xl font-black tracking-tight mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          <div className={`backdrop-blur-md border-[2px] rounded-[22px] p-6 transition-all duration-300 hover:scale-[1.02] ${
            isDark ? 'bg-[#24262B]/85 border-[#3A3F4A] hover:border-[#0A44FF]' : 'bg-white/85 border-[#E2E6F0] hover:border-[#0A44FF]'
          }`}>
          ...
          <div className={`backdrop-blur-md border-[2px] rounded-[22px] p-6 transition-all duration-300 hover:scale-[1.02] ${
            isDark ? 'bg-[#24262B]/85 border-[#3A3F4A] hover:border-[#FF7A2F]' : 'bg-white/85 border-[#E2E6F0] hover:border-[#FF7A2F]'
          }`}>
          ...
          <div className={`backdrop-blur-md border-[2px] rounded-[22px] p-6 transition-all duration-300 hover:scale-[1.02] ${
            ...
            <Mic className="w-8 h-8 text-[#0A44FF] mb-3" />
          ...
          <div className={`backdrop-blur-md border-[2px] rounded-[22px] p-6 transition-all duration-300 hover:scale-[1.02] ${
            ...
            <Globe className="w-8 h-8 text-[#FF7A2F] mb-3" />
        ...
        <div className={`border rounded-[26px] p-6 md:p-10 shadow-xl backdrop-blur-md ${
          isDark ? 'bg-[#24262B]/90 border-[#3A3F4A]' : 'bg-white/95 border-[#E2E6F0]'
        }`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className={`border rounded-2xl p-5 relative ${isDark ? 'bg-[#1C1C1E] border-[#3A3F4A]' : 'bg-gray-50 border-gray-200'}`}>
              ...
              <h4 className={`text-base font-bold mb-1.5 ${isDark ? 'text-white' : 'text-gray-900'}`}>Web Audio & Speech Bridge</h4>
              <p className={`text-xs m-0 leading-relaxed font-medium ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                If voice command or TTS is requested, `SpeechRecognition` processes interim chunks with fuzzy Levenshtein scoring while the Web Audio API monitors ambient audio levels.
              </p>
            </div>
      ...
      {/* ======================================================================
          ...
          ====================================================================== */}
      <section id="metrics" className={`py-20 md:py-28 px-4 md:px-8 max-w-7xl mx-auto border-t ${
        isDark ? 'border-[#3A3F4A]/60' : 'border-gray-200'
      }`}>
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className={`text-3xl md:text-4xl lg:text-5xl font-black tracking-tight mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className={`backdrop-blur-md border-[2px] rounded-[22px] p-6 text-center shadow-lg transition-all duration-300 hover:scale-[1.02] ${
            isDark ? 'bg-[#24262B]/85 border-[#3A3F4A] hover:border-[#FF7A2F]' : 'bg-white/85 border-[#E2E6F0] hover:border-[#FF7A2F]'
          }`}>
      ...
      {/* ======================================================================
          ...
          ====================================================================== */}
      <section id="guide" className={`py-20 md:py-28 px-4 md:px-8 max-w-7xl mx-auto border-t ${
        isDark ? 'border-[#3A3F4A]/60' : 'border-gray-200'
      }`}>
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className={`text-3xl md:text-4xl lg:text-5xl font-black tracking-tight mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className={`backdrop-blur-md border-[2px] rounded-[22px] p-6 md:p-8 flex flex-col justify-between transition-all duration-300 hover:scale-[1.02] ${
            isDark ? 'bg-[#24262B]/85 border-[#3A3F4A] hover:border-[#0A44FF]' : 'bg-white/85 border-[#E2E6F0] hover:border-[#0A44FF]'
          }`}>
            <div>
              <div className="flex items-center justify-between mb-6">
                <span className={`w-10 h-10 rounded-xl font-black text-lg flex items-center justify-center border ${
                  isDark ? 'bg-[#0A44FF]/22 text-[#6AA2FF] border-[#0A44FF]/40' : 'bg-[#0A44FF]/12 text-[#0A44FF] border-[#0A44FF]/30'
                }`}>
                  1
                </span>
          ...
          <div className={`backdrop-blur-md border-[2px] rounded-[22px] p-6 md:p-8 flex flex-col justify-between transition-all duration-300 hover:scale-[1.02] ${
            isDark ? 'bg-[#24262B]/85 border-[#3A3F4A] hover:border-purple-500' : 'bg-white/85 border-[#E2E6F0] hover:border-purple-500'
          }`}>
            <div>
              <div className="flex items-center justify-between mb-6">
      ...
      {/* ======================================================================
          ...
          ====================================================================== */}
      <section id="video" className={`py-20 md:py-28 px-4 md:px-8 max-w-7xl mx-auto border-t ${
        isDark ? 'border-[#3A3F4A]/60' : 'border-gray-200'
      }`}>
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className={`text-3xl md:text-4xl lg:text-5xl font-black tracking-tight mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>
        ...
        <div className={`relative max-w-4xl mx-auto border-2 border-dashed rounded-[26px] p-8 md:p-16 text-center shadow-2xl overflow-hidden group ${
          isDark ? 'bg-[#24262B]/85 border-[#0A44FF]/50' : 'bg-white/90 border-[#0A44FF]/40'
        }`}>
      ...
      {/* ======================================================================
          ...
          ====================================================================== */}
      <section id="team" className={`py-20 md:py-28 px-4 md:px-8 max-w-7xl mx-auto border-t ${
        isDark ? 'border-[#3A3F4A]/60' : 'border-gray-200'
      }`}>
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className={`text-3xl md:text-4xl lg:text-5xl font-black tracking-tight mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          <div className={`backdrop-blur-md border-[2px] rounded-[22px] p-6 text-center flex flex-col items-center justify-between transition-all duration-300 hover:scale-[1.02] ${
            isDark ? 'bg-[#24262B]/85 border-[#3A3F4A] hover:border-[#0A44FF]' : 'bg-white/85 border-[#E2E6F0] hover:border-[#0A44FF]'
          }`}>
            <div className={`w-20 h-20 rounded-full border flex flex-col items-center justify-center text-[#0A44FF] mb-4 shadow-inner ${
              isDark ? 'bg-[#1C1C1E] border-[#3A3F4A]' : 'bg-gray-100 border-gray-300'
            }`}>
            ...
            <div>
              <h3 className={`text-lg font-extrabold m-0 ${isDark ? 'text-white' : 'text-gray-900'}`}>[ PLACEHOLDER: Name ]</h3>
              <span className={`inline-block px-2.5 py-1 rounded-full text-[11px] font-bold border my-2 ${
                isDark ? 'bg-[#0A44FF]/20 text-[#6AA2FF] border-[#0A44FF]/40' : 'bg-[#0A44FF]/12 text-[#0A44FF] border-[#0A44FF]/30'
              }`}>
                Project Lead / Full-Stack Dev
              </span>
              <p className={`text-xs leading-relaxed m-0 mt-1 font-medium ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                Architected the Plasmo extension framework, React 18 UI/UX design tokens, and state synchronization across browser tabs.
              </p>
            </div>
          </div>
          ...
          <div className={`backdrop-blur-md border-[2px] rounded-[22px] p-6 text-center flex flex-col items-center justify-between transition-all duration-300 hover:scale-[1.02] ${
            ...
            <div className={`w-20 h-20 rounded-full border flex flex-col items-center justify-center text-[#FF7A2F] mb-4 shadow-inner ${
              isDark ? 'bg-[#1C1C1E] border-[#3A3F4A]' : 'bg-gray-100 border-gray-300'
            }`}>
              <Users size={26} className="mb-1" />
              <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400">[ PHOTO ]</span>
            ...
            <div>
              <h3 className={`text-lg font-extrabold m-0 ${isDark ? 'text-white' : 'text-gray-900'}`}>[ PLACEHOLDER: Name ]</h3>
              <span className={`inline-block px-2.5 py-1 rounded-full text-[11px] font-bold border my-2 ${
                isDark ? 'bg-[#FF7A2F]/20 text-[#FFC09B] border-[#FF7A2F]/40' : 'bg-[#FF7A2F]/12 text-[#FF7A2F] border-[#FF7A2F]/30'
              }`}>
                AI & Voice Systems Engineer
              </span>
              <p className={`text-xs leading-relaxed m-0 mt-1 font-medium ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                Engineered the Web Speech API bridge, Levenshtein fuzzy scoring algorithm, and audio waveform frequency visualizer.
              </p>
            </div>
          </div>
          ...
          {/* Adviser Card Highlight */}
          <div className={`backdrop-blur-md border-2 rounded-[22px] p-6 text-center flex flex-col items-center justify-between transition-all duration-300 ${
            isDark 
              ? 'bg-[#24262B]/95 border-yellow-400/80 shadow-[0_0_25px_rgba(250,204,21,0.2)]' 
              : 'bg-white border-yellow-500 shadow-[0_8px_24px_rgba(250,204,21,0.2)]'
          }`}>
            <div className={`w-20 h-20 rounded-full border border-yellow-400/50 flex flex-col items-center justify-center text-yellow-500 mb-4 shadow-inner ${
              isDark ? 'bg-[#1C1C1E]' : 'bg-yellow-50'
            }`}>
              <Award size={26} className="mb-1" />
              <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400">[ PHOTO ]</span>
            </div>
            <div>
              <h3 className={`text-lg font-extrabold m-0 ${isDark ? 'text-white' : 'text-gray-900'}`}>[ PLACEHOLDER: Adviser Name ]</h3>
              <span className="inline-block px-2.5 py-1 rounded-full text-[11px] font-bold bg-yellow-400/20 text-yellow-500 border border-yellow-400/50 my-2">
                Capstone Thesis Adviser
              </span>
              <p className={`text-xs leading-relaxed m-0 mt-1 font-medium ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                Provided technical guidance, academic mentoring, and research verification for the Sensa Capstone Project at BulSU.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* ======================================================================
          ...
          ====================================================================== */}
      <footer className={`border-t pt-16 pb-12 px-4 md:px-8 ${
        isDark ? 'bg-[#1C1C1E] border-[#3A3F4A]' : 'bg-white border-gray-200'
      }`}>
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div>
            ...
            <div className={`text-xs leading-relaxed p-4 rounded-xl border font-medium ${
              isDark ? 'bg-[#24262B] border-[#3A3F4A] text-gray-400' : 'bg-gray-100 border-gray-300 text-gray-700'
            }`}>
              🔒 <strong className={isDark ? 'text-white' : 'text-gray-900'}>Local Processing Guarantee:</strong> All voice commands (`SpeechRecognition`) and audio transcriptions operate locally within the user's browser. Sensa does not collect, store, or transmit any audio data to external third-party cloud servers.
            </div>
          </div>
        </div>

        <div className={`max-w-7xl mx-auto pt-8 border-t flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-semibold ${
          isDark ? 'border-[#3A3F4A] text-gray-400' : 'border-gray-200 text-gray-600'
        }`}>
      ...
      {activeMode === 'visual' && (
        <div className="fixed bottom-4 left-4 md:left-8 z-50 animate-pop transition-all duration-300">
          <div className={`border-2 rounded-[22px] p-2.5 shadow-[0_12px_36px_rgba(10,68,255,0.45)] backdrop-blur-md transition-all duration-300 flex flex-col gap-3 items-center ${
            isDark ? 'bg-[#24262B]/90 border-[#0A44FF]' : 'bg-white/90 border-[#0A44FF]'
          }`}>
            ...
            <button
              onClick={() => handleVoiceSimulator('Read Page Content')}
              className={`w-11 h-11 rounded-2xl flex items-center justify-center hover:scale-[1.02] active:scale-95 transition-all duration-200 focus:ring-2 focus:ring-[#0A44FF] focus:outline-none cursor-pointer ${
                ttsPlaying 
      ...
      {activeMode === 'auditory' && (
        <div className="fixed bottom-4 right-4 md:right-8 z-50 max-w-[calc(100vw-2rem)] sm:max-w-sm w-full animate-pop transition-all duration-300">
          <div className={`border-2 rounded-[22px] p-4 shadow-[0_12px_36px_rgba(255,122,47,0.45)] backdrop-blur-md transition-all duration-300 ${
            isDark ? 'bg-[#24262B]/90 border-[#FF7A2F]' : 'bg-white/90 border-[#FF7A2F]'
          }`}>
